#version 330 core

out vec4 fragColor;

uniform vec2 resolution;
uniform sampler2D scene;
// 0 to 0.5
uniform float curve;
// -0.5 to 0.5
uniform float color_offset;
// 0 to 1
uniform float mask_brightness;
/*
Set the type of mask this CRT will have.
Dots: emulates a typical PC CRT monitor. (1)
Grille: emulates an aperture grille. Good with higher curve values. (2)
Wide Grille: more suitable for 4k monitors. (3)
Soft Grille: very close to wide grille, but a little softer. (4)
Slot mask: this is the pattern found on most TV sets and arcade cabnets (5)
*/
uniform int mask_type = 1;
/*
This should be the input texture's height divided by width.
Only important if curve is used.
For 16:9, this should be 0.5625.
*/
uniform float aspect;

vec2 warp(vec2 uv, float _aspect, float _curve)
{
    uv -= 0.5;
    uv.x /= _aspect;
    float warping = dot(uv, uv) * _curve;
    warping -= _curve * 0.25;
    uv /= 1.0 - warping;
    uv.x *= _aspect;
    uv += 0.5;
    return uv;
}

vec3 linear_to_srgb(vec3 col)
{
    return mix(
        (pow(col, vec3(1.0 / 2.4)) * 1.055) - 0.055,
        col * 12.92,
        lessThan(col, vec3(0.0031318))
    );
}

vec3 srgb_to_linear(vec3 col)
{
    return mix(
        pow((col + 0.055) / 1.055, vec3(2.4)),
        col / 12.92,
        lessThan(col, vec3(0.04045))
    );
}

vec3 get_sample(vec2 uv)
{
    uv *= vec2(textureSize(scene, 0));
    int y = int(uv.y + 0.5) - 1;
    float x = floor(uv.x);
    float ax = x - 1.0;
    float dx = x + 1.0;
    vec3 upper_a = texelFetch(scene, ivec2(int(ax), y), 0).rgb;
    vec3 upper_b = texelFetch(scene, ivec2(int(x), y), 0).rgb;
    vec3 upper_c = texelFetch(scene, ivec2(int(dx), y), 0).rgb;
    y += 1;
    vec3 lower_a = texelFetch(scene, ivec2(int(ax), y), 0).rgb;
    vec3 lower_b = texelFetch(scene, ivec2(int(x), y), 0).rgb;
    vec3 lower_c = texelFetch(scene, ivec2(int(dx), y), 0).rgb;
    upper_a = srgb_to_linear(upper_a);
    upper_b = srgb_to_linear(upper_b);
    upper_c = srgb_to_linear(upper_c);
    lower_a = srgb_to_linear(lower_a);
    lower_b = srgb_to_linear(lower_b);
    lower_c = srgb_to_linear(lower_c);
    vec3 beam = vec3(uv.x - 0.5);
    beam.r -= color_offset;
    beam.b += color_offset;
    vec3 weight_a = smoothstep(1, 0, (beam - ax));
    vec3 weight_b = smoothstep(1, 0, abs(beam - x));
    vec3 weight_c = smoothstep(1, 0, (dx - beam));
    vec3 upper_col = (
        upper_a * weight_a +
            upper_b * weight_b +
            upper_c * weight_c
        );
    vec3 lower_col = (
        lower_a * weight_a +
            lower_b * weight_b +
            lower_c * weight_c
        );
    float sawtooth = (uv.y + 0.5) - float(y);
    sawtooth = smoothstep(0, 1, sawtooth);
    vec3 combined = mix(upper_col, lower_col, sawtooth);
    return combined;
}

vec4 generate_mask(vec2 fragcoord) {
    switch (mask_type) {
        case 1:
        { // Dots
            vec3 pattern[4] = vec3[](vec3(1, 0, 0), vec3(0, 1, 0), vec3(0, 0, 1), vec3(0, 0, 0));
            ivec2 icoords = ivec2(fragcoord);
            return vec4(pattern[(icoords.y * 2 + icoords.x) % 4], 0.33);
        }

        case 2:
        { // Grille
            vec3 pattern[2] = vec3[](vec3(0, 1, 0), vec3(1, 0, 1));
            return vec4(pattern[int(fragcoord.x) % 2], 0.5);
        }

        case 3:
        { // Wide grille
            vec3 pattern[4] = vec3[](vec3(1, 0, 0), vec3(0, 1, 0), vec3(0, 0, 1), vec3(0, 0, 0));
            return vec4(pattern[int(fragcoord.x) % 4], 0.25);
        }

        case 4:
        { // Grille wide soft
            vec3 pattern[4] = vec3[](
                    vec3(1.0, 0.125, 0.0),
                    vec3(0.125, 1.0, 0.125),
                    vec3(0.0, 0.125, 1.0),
                    vec3(0.125, 0.0, 0.125)
                );
            return vec4(pattern[int(fragcoord.x) % 4], 0.3125);
        }

        case 5:
        { // Slotmask
            vec3 pattern[16] = vec3[](
                    vec3(1, 0, 1), vec3(0, 1, 0), vec3(1, 0, 1), vec3(0, 1, 0),
                    vec3(0, 0, 1), vec3(0, 1, 0), vec3(1, 0, 0), vec3(0, 0, 0),
                    vec3(1, 0, 1), vec3(0, 1, 0), vec3(1, 0, 1), vec3(0, 1, 0),
                    vec3(1, 0, 0), vec3(0, 0, 0), vec3(0, 0, 1), vec3(0, 1, 0)
                );
            ivec2 icoords = ivec2(fragcoord) % 4;
            return vec4(pattern[icoords.y * 4 + icoords.x], 0.375);
        }

        default:
        return vec4(0.5, 0.5, 0.5, 0.5);
    }
}

vec3 apply_mask(vec3 linear_color, vec2 fragcoord)
{
    vec4 mask = generate_mask(fragcoord);
    linear_color *= mix(mask.w, 1.0, mask_brightness);
    vec3 target_color = linear_color / mask.w;
    vec3 primary_col = clamp(target_color, 0.0, 1.0);
    vec3 secondary = target_color - primary_col;
    secondary /= 1.0 / mask.w - 1.0;
    primary_col *= mask.rgb;
    primary_col += secondary * (1.0 - mask.rgb);
    return primary_col;
}

vec2 get_uv(vec2 pos) {
    return pos / resolution;
}

void main() {
    vec2 uv = get_uv(gl_FragCoord.xy);
    vec2 warped_coords = warp(uv, aspect, curve);
    vec3 col = get_sample(warped_coords);
    col = apply_mask(col, gl_FragCoord.xy);
    col = linear_to_srgb(col);
    vec4 original = texture(scene, uv);
    fragColor.rgb = col;
    fragColor.a = original.a;
}

